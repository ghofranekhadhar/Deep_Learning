"""
╔══════════════════════════════════════════════════════════╗
║   ANIME EDUCATIF STUDIO — Streamlit + Groq (GRATUIT)   ║
...
@st.cache_resource
def install_deps():
    deps = ["groq", "edge-tts", "gtts", "pydub", "opencv-python-headless", "pillow", "numpy"]
    for pkg in deps:
...
#  SCENES
# ══════════════════════════════════════════════════════════
def build_scenes(char, song):
    p = char.prenom
    f = Config.FPS
    return [
        Scene("Introduction",      "Intro",                   "parc",   "saute_joie",        "heureux",   f"La chanson de {p} !",             f*5, "day",    "intro"),
        Scene("La vie normale",    "Acte I",                  "parc",   "court_vite",        "heureux",   f"{p} joue, rit !",                 f*5, "day",    "acte1"),
        Scene("Belle journée",     "Acte I",                  "maison", "marche_content",    "heureux",   f"{p} est heureux.",                f*4, "golden", "acte1"),
        Scene("Oh qu'est-ce ?",    "Acte II",                 "maison", "decouvre_surpris",  "curieux",   f"Quelque chose attire {p}...",     f*5, "golden", "acte2"),
        Scene("Une idée...",       "Acte II",                 "maison", "hesite_balance",    "penseur",   f"Juste une petite fois...",        f*4, "golden", "acte2"),
        Scene("ATTENTION !",       "Refrain - Avertissement", "parc",   "appelle_gestes",    "effraye",   f"Non non non ! Danger !",          f*5, "day",    "refrain1"),
        Scene("NON NON NON !",     "Refrain - Avertissement", "parc",   "saute_peur",        "effraye",   f"Appelle un adulte !",             f*4, "day",    "refrain1"),
        Scene("La bêtise !",       "Acte III",                "maison", "fait_betise_saute", "curieux",   f"{p} n'écoute pas !",             f*6, "dusk",   "acte3"),
        Scene("Conséquences !",    "Acte IV",                 "danger", "court_panique",     "effraye",   f"{p} a très peur !",              f*6, "dusk",   "acte4"),
        Scene("AU SECOURS !",      "Acte IV",                 "danger", "appelle_gestes",    "effraye",   f"MAMAN ! PAPA !",                 f*5, "dusk",   "acte4"),
        Scene("La leçon",          "Refrain - Leçon",         "parc",   "ecoute_hoche",      "desole",    f"Voilà ce qu'il faut faire.",     f*5, "day",    "refrain2"),
        Scene(f"{p} comprend",     "Acte V",                  "parc",   "pleure_assise",     "triste",    f"{p} pleure... il comprend.",     f*6, "day",    "acte5"),
        Scene("La promesse",       "Acte VI",                 "parc",   "saute_promesse",    "determine", f"{p} fait une promesse !",        f*5, "day",    "acte6"),
        Scene("Et toi ?",          "Message final",           "parc",   "pointe_enfant",     "heureux",   f"{p} te parle !",                 f*5, "day",    "outro"),
        Scene("À bientôt !",       "Message final",           "parc",   "salue_saute",       "fier",      f"Bravo d'avoir regardé !",        f*4, "day",    "outro"),
    ]
...
class P:
    ...
    SUN=(255,245,100);     FLAME_O=(255,110,0)
    FLAME_C=(255,230,80);  TEAR=(90,195,255)
...
def draw_bg(draw,decor,mood,frame):
    ...
    draw.rectangle([0,int(S*.64),S,S],fill=P.GRASS_S if mood=="dusk" else P.GRASS)
    if mood!="dusk":
        for i in range(5):
            fx=40+i*int(S/5)+int(4*math.sin(frame*.02+i)); fy=int(S*.645)
            for pp in range(5):
    ...
    draw.rectangle([tx-8,ty-int(S*.22),tx+8,ty],fill=(85,52,28))
    draw.ellipse([tx-int(S*.09),ty-int(S*.32),tx+int(S*.09),ty-int(S*.16)],fill=(60,185,60))
    if decor in("maison","danger"):
        mx,my=30,int(S*.62); mw,mh=int(S*.28),int(S*.22)
        draw.rectangle([mx,my-mh,mx+mw,my],fill=P.WALL,outline=P.OUTLINE,width=2)
        draw.polygon([mx-10,my-mh,mx+mw//2,my-mh-int(S*.09),mx+mw+10,my-mh],fill=P.ROOF,outline=P.OUTLINE,width=2)
        draw.rectangle([mx+mw//2-15,my-int(S*.1),mx+mw//2+15,my],fill=P.DOOR,outline=P.OUTLINE,width=2)
        wl=(255,255,180) if mood=="dusk" else P.WINDOW
        for wx2 in[mx+8,mx+mw-38]:
            draw.rectangle([wx2,my-int(S*.17),wx2+30,my-int(S*.1)],fill=wl,outline=P.OUTLINE,width=2)
        if mood=="dusk":
            for fi in range(5):
                fh2=30+int(18*math.sin(frame*.22+fi*.8)); fx2=mx+10+fi*int(mw/5)
...
def anim_off(action,frame):
    ...
    if action=="pleure_assise": return 0,int(Config.SIZE*.04)
    return 0,int(3*math.sin(frame*.07))

def draw_char(draw,cx,cy,action,emotion,frame,genre):
    S=Config.SIZE
    dx,dy=anim_off(action,frame); x,y=cx+dx,cy+dy
    ...
    draw.ellipse([cx-int(S*.06),cy+int(S*.015),cx+int(S*.06),cy+int(S*.03)],fill=(0,0,0))
...
def get_fonts():
    ...
    try:
        S=Config.SIZE
        _FONTS={"big":ImageFont.truetype(Config.FONT_BOLD,max(16,S//20)),
...
def draw_ui(img,scene,f_in_scene,song,genre):
    draw=ImageDraw.Draw(img); F=get_fonts(); S=Config.SIZE
    ov=Image.new("RGBA",(S,50),(0,0,0,0)); d2=ImageDraw.Draw(ov)
    for yy in range(50):
        alpha=int(210*(1-yy/50)); d2.line([(0,yy),(S,yy)],fill=(*P.UI_BG,alpha))
    img.paste(Image.alpha_composite(img.crop((0,0,S,50)).convert("RGBA"),ov),(0,0))
    draw=ImageDraw.Draw(img)
    draw.text((12,8),scene.titre[:40],fill=P.UI_TFG,font=F["med"])
    bh=int(S*.22)
    ov2=Image.new("RGBA",(S,bh),(0,0,0,0)); d3=ImageDraw.Draw(ov2)
    for yy in range(bh):
        alpha=int(230*(yy/bh)**.4); d3.line([(0,yy),(S,yy)],fill=(*P.SONG_BG,alpha))
...
def render_scene(scene,genre,song,gframe):
    ...
    for f in range(scene.duree):
        ...
        draw_char(draw,Config.SIZE//2,int(Config.SIZE*.58),scene.action,scene.emotion,gframe+f,genre)
...
def render_all(scenes,genre,song,progress_bar):
    all_frames=[]; gframe=0; EF=Config.EASE_FRAMES; total=len(scenes)
...
def gen_audio(char,song,folder,status_text):
    sections=[song.intro,"...",song.acte1,"...",song.acte2,"...",
    ...
    vpath=os.path.join(folder,"voix.mp3"); ok=False
    if _EDGE_TTS_OK:
        try:
...
def encode_video(frames,audio_path,folder,prenom):
    ...
    writer.release()
    subprocess.run(["ffmpeg","-y","-i",silent,"-i",audio_path,
...
# ══════════════════════════════════════════════════════════
def main():
    st.set_page_config(page_title="Studio Animé Éducatif", page_icon="🎬", layout="centered")
    ...
    with st.sidebar:
        ...
        api_key=st.text_input("Clé API Groq (100% GRATUIT)",type="password",
                               placeholder="gsk_...",help="Obtiens ta clé sur console.groq.com")
        st.markdown("---")
        st.markdown("**Obtenir la clé gratuite :**")
        st.markdown("1. Va sur [console.groq.com](https://console.groq.com)")
        st.markdown("2. Crée un compte (Google ou email)")
        st.markdown("3. Clique **API Keys** → **Create API Key**")
        st.markdown("4. Copie et colle la clé ici ⬆️")
        st.markdown("---")
        st.caption("🔒 La clé n'est jamais sauvegardée.")

    col1,col2=st.columns([3,1])
    with col1:
        probleme=st.text_area("La bêtise de ton enfant",
            placeholder="Ex: Mon fils Adam, 5 ans, touche les prises électriques avec ses doigts",height=100)
    with col2:
        st.markdown("<br>",unsafe_allow_html=True)
        st.markdown("**Exemples :**")
        exemples={
            "⚡ Prises":      "Mon fils Adam, 5 ans, touche les prises électriques",
            "🔪 Couteaux":    "Mon fils Youssef, 7 ans, joue avec les couteaux de cuisine",
            "💊 Médicaments": "Ma fille Inès, 6 ans, mange des médicaments dans l'armoire",
            "🏊 Piscine":     "Ma fille Lina, 4 ans, s'approche seule du bord de la piscine",
            "🚗 Route":       "Mon fils Rayan, 6 ans, traverse la rue sans regarder",
        }
        for label,texte in exemples.items():
            if st.button(label,use_container_width=True):
                st.session_state["exemple"]=texte; st.rerun()

    if "exemple" in st.session_state:
        probleme=st.session_state.pop("exemple"); st.rerun()

    if st.button("🎬 Générer la vidéo",type="primary",use_container_width=True):
        if not api_key:
            st.error("⚠️ Entre ta clé API Groq dans la barre latérale ⚙️ — c'est gratuit !"); st.stop()
        if not probleme.strip():
            st.error("⚠️ Écris la bêtise de ton enfant avant de générer."); st.stop()

        with st.status("🎬 Génération en cours...",expanded=True) as status:
            st.write("🤖 L'IA génère le scénario personnalisé...")
            try:
                char,song,danger=generate_with_ai(probleme,api_key)
            except json.JSONDecodeError:
                st.error("L'IA n'a pas retourné un JSON valide. Réessaie."); st.stop()
            except Exception as e:
                st.error(f"Erreur API Groq : {e}"); st.stop()
            ...
            with st.expander("📜 Voir la chanson générée",expanded=False):
                for label,texte in [
                    ("🎵 Intro",song.intro),("Acte 1 — Vie normale",song.acte1),
                    ("Acte 2 — La tentation",song.acte2),("🚨 Refrain 1",song.refrain1),
                    ("⚠️ Acte 3 — La bêtise",song.acte3),("🔥 Acte 4 — Conséquence",song.acte4),
                    ("💡 Refrain 2",song.refrain2),("😢 Acte 5 — Compréhension",song.acte5),
                    ("🤝 Acte 6 — La promesse",song.acte6),("🫵 Outro",song.outro)]:
                    st.markdown(f"**{label}**"); st.caption(texte)
        ...
        st.download_button(label="💾 Télécharger la vidéo MP4",data=video_bytes,
            file_name=f"anime_{char.prenom.lower()}_{danger.replace(' ','_')}.mp4",
            mime="video/mp4",use_container_width=True)

    st.markdown("---")
    st.caption("Studio Animé Éducatif · Propulsé par Groq AI (gratuit) · Pour les enfants de 3 à 8 ans")
